...
def parse_case_html(html: str) -> Tuple[List[Dict], List[str]]:
    ...
    # CSS class → 稀有度英文（按CS2颜色规则）
    rarity_class_map = {
        'common': 'Consumer',      # 灰
        'uncommon': 'Industrial',  # 浅蓝
        'rare': 'Mil-Spec',        # 蓝
        'mythical': 'Restricted',  # 紫
        'legendary': 'Classified', # 粉紫
        'ancient': 'Covert',       # 红
        'gold-rare': 'Gold',       # 金（有的页面用 gold-rare）
        'gold': 'Gold',            # 金（有的页面用 gold）
    }
    ...
    for g in galleries:
        ...
        # 获取稀有度
        # 选择 <span class="..."> 的方式要能覆盖 gold-rare
        ...
        # ✅ 关键修复：不要把 "Covert/Restricted/..." 文本拼进物品名
        ...
        if parts and parts[0].strip().lower() in (
            ...
            "common","uncommon","rare","mythical","legendary","ancient","gold-rare"
        ):
            parts = parts[1:]
...
def enrich_with_steamdt(df: pd.DataFrame, api_key: str) -> pd.DataFrame:
    """使用 SteamDT API 获取 marketHashName 和中文名称"""
    ...
    # 创建 marketHashName -> 中文名 的映射
    name_zh_map = {}
    for _, row in base.iterrows():
        mhn = row.get("marketHashName", "")
        name_zh = row.get("name", "")  # SteamDT 的 'name' 字段就是中文名
        if mhn and name_zh:
            name_zh_map[mhn] = name_zh
    ...
    def match(row):
        ...
        if len(hit) > 0:
            mhn = hit.iloc[0]["marketHashName"]
            return mhn, name_zh_map.get(mhn, "")
        return "", ""
    
    # 应用匹配，同时获取 marketHashName 和中文名
    df[["steamdt_market_hash_name", "item_name_zh"]] = df.apply(
        lambda r: pd.Series(match(r)), axis=1
    )
    return df

# ---------- Main flow ----------
def main(out_items="cs2_case_items_full.csv", out_cases="cs2_cases.csv", link_steamdt=True):
    # 1) 拉所有武器箱页面
    ...
    if link_steamdt:
        ...
        if api_key:
            try:
                ...
                print(f"⚠️ SteamDT 获取中文名称失败: {e}")
        else:
            print("⚠️ 警告: 未设置 STEAMDT_API_KEY，跳过中文名称获取。")
